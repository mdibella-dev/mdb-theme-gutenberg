...
function recalcMasonry( masonry ) {
    const masonry_elements = document.querySelectorAll( '.masonry' );

    masonry_elements.forEach( masonry => {
...
/**
 * Event handler:
 * - on resize: recalc the masonry
 * - on readystatechange:
 */

// on resize
window.addEventListener( 'resize', (event) => { recalcMasonry(); } );
